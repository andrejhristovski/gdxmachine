...
class NormalMapRenderer : Renderer {
    ...
    init {
        ...
        val vertexAttributes = VertexAttributes(
                ...
                VertexAttribute(Usage.TextureCoordinates, 2, "${ShaderProgram.TEXCOORD_ATTRIBUTE}_normal")
        )
        ...
        shaderProgram = shaderFactory.get("normal_map", "normal_map")
    }
    ...
    override fun draw(drawable: Drawable) {
        val texture = (drawable as Sprite).getTexture().texture
        var normal: Texture? = null
        if (drawable.getNormalMap() !== null) normal = drawable.getNormalMap()!!.texture
        validateCachedTexture(texture, normal)
    ...
    private fun appendVertices(sprite: Sprite) {
        ...
        vertices[idx + 5] = if (sprite.getNormalMap() !== null) sprite.getNormalMap()!!.u else 0f
        vertices[idx + 6] = if (sprite.getNormalMap() !== null) sprite.getNormalMap()!!.v2 else 0f
        ...
        vertices[idx + 12] = if (sprite.getNormalMap() !== null) sprite.getNormalMap()!!.u else 0f
        vertices[idx + 13] = if (sprite.getNormalMap() !== null) sprite.getNormalMap()!!.v else 0f
        ...
        vertices[idx + 19] = if (sprite.getNormalMap() !== null) sprite.getNormalMap()!!.u2 else 0f
        vertices[idx + 20] = if (sprite.getNormalMap() !== null) sprite.getNormalMap()!!.v else 0f
        ...
        vertices[idx + 26] = if (sprite.getNormalMap() !== null) sprite.getNormalMap()!!.u2 else 0f
        vertices[idx + 27] = if (sprite.getNormalMap() !== null) sprite.getNormalMap()!!.v2 else 0f
    }
    ...
    private fun flush() {
        ...
        if (normalExist == 1) cachedNormal!!.bind(1)
    ...
    private fun validateCachedTexture(texture: Texture, normal: Texture?) {
        if (cachedTexture !== texture || cachedNormal !== normal) {
            if (bufferedCalls > 0) flush()
