...
/** The goal behind the Scene Module is to take care of the scenes & scene lifecycle.
 * Scene Module provides API for managing scenes, and take care for disposing everything from the previous scene when
 * the scene is changed */
class SceneModule : Core.Module {

    class Api(private val sceneModule: SceneModule) : Core.Api {

        /** Scene.Api#set() can be used to change the current scene to another state.
         * After invoking this method the next scene is scheduled to be the next.
         * The change of the state is not invoked immediately */
    ...
    /** SceneModule#unload() is invoked by the main lifecycle for unloading the scene module */
    override fun unload() {
        currentScene = null
        nextScene = null
    }
